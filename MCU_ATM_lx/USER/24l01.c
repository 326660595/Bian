...
///////////////////////////////////////////////////////lllllllllllllllllllxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
//启动NRF24L01发送一次数据
//txbuf:待发送数据首地址
//返回值:发送完成状况
...
void fs(int fk,u8 hour,u8 minute,u8 time,int ye,int jg)
{
	printf("\n\rName:ZhangSan  \r");
			printf("\rPay: %d\r",fk);printf("\r  Time: %d\r",hour);printf("\r %d\r",minute);
			printf("\r %d\r",time);printf("\r  Balance: %d\r",ye);printf("\r Result: %d\r",jg);
			//printf("%d %d %d\r",hour,minute,time);
}
//启动NRF24L01发送一次数据
//txbuf:待发送数据首地址
//返回值:0，接收完成；其他，错误代码
